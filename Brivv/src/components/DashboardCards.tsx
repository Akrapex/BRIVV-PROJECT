...
function DashboardCards({text, count, icon, plusSign, rate, indicator}: Stats) {
  return (
    <article className="bg-white p-6 w-55 border border-[#CFDBE7] rounded-xl grow">
        ...
        <p className="text-[24px] font-bold leading-8 text-[#0D141B]">{count}</p> <span className={`flex items-center text-[14px] font-semibold leading-5 ${rate > 0 ? "text-[#078838]" : "text-[#4C739A]"}`}>{icon}{rate > 0 && plusSign}{rate}{indicator}</span>
        </div>
    </article>
  )
}
